...
export function Dijkstra(Grid, startNode, endNode, N, M) {
  var visitedNodes = [];
  var path = [];
  ...
  while (priority_queue.length > 0) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (x === endNode.x && y === endNode.y) {
        ...
        grid[x][y].distance >
        Grid[x][y].weight + grid[top.x][top.y].distance
      ) {
        grid[x][y].distance = Grid[x][y].weight + grid[top.x][top.y].distance;
        grid[x][y].x = top.x;
        grid[x][y].y = top.y;
